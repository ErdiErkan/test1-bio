...
// REPORT_TYPES dizisini bileşen içine taşıdık çünkü çeviri hook'unu kullanacağız
const REPORT_TYPE_KEYS: ReportType[] = ['WRONG_INFO', 'TYPO', 'IMAGE_ISSUE', 'OTHER']
...
export default function ReportForm({ celebrityId, onSuccess }: ReportFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (message.trim().length < 10) {
      addToast(t('error_min_length'), 'error') // ✅ Çeviri
      return
    }
    ...
    startTransition(async () => {
      ...
      if (result.success) {
        addToast(t('success'), 'success') // ✅ Çeviri
        setMessage('')
        setEmail('')
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-2">
          {REPORT_TYPE_KEYS.map((typeKey) => (
            ...
            >
              ...
              />
              <div className="ml-3">
                <span className="text-sm font-medium text-gray-900">{t(`types.${typeKey}`)}</span>
                <p className="text-xs text-gray-500 mt-0.5">{t(`types.${typeKey}_DESC`)}</p>
              </div>
            </label>
          ))}
        </div>
      ...
      <div>
        ...
        <div className="flex justify-between mt-1">
          {isMessageTooShort && (
            <span className="text-xs text-red-500">{t('error_min_length')}</span>
          )}
          {isMessageTooLong && (
            <span className="text-xs text-red-500">{t('error_max_length')}</span>
          )}
          {!isMessageTooShort && !isMessageTooLong && <span />}
          <span className={`text-xs ${characterCount > 900 ? 'text-orange-500' : 'text-gray-400'}`}>
            {t('chars', { count: characterCount })}
          </span>
        </div>
      </div>
      ...
      <div>
        <label htmlFor="email" className="block text-sm font-medium text-gray-700 mb-1">
          {t('email')} <span className="text-gray-400 text-xs">{t('email_optional')}</span>
        </label>
        <input
          type="email"
          id="email"
