...
// ✅ DÜZELTME: Artık async bir Server Component
export default async function CelebrityHeader({ celebrity }: CelebrityHeaderProps) {
  // ✅ DÜZELTME: useTranslations yerine getTranslations (async)
  const t = await getTranslations('celebrity')
  // ✅ DÜZELTME: useLocale yerine getLocale (async)
  ...
  // Fetch Rankings from Redis/Cache (Architecture Phase 4)
  const rankings = await getDynamicRankings(celebrity.id, locale, {
    categorySlug: celebrity.categories?.[0]?.slug, // Assuming first category until we have primary
    zodiac: zodiac?.sign,
    birthYear: birthYear
  });
    ...
    <div className="bg-white rounded-lg shadow-lg overflow-hidden">
      <div className="md:flex">
        ...
        <div className="md:w-2/3 p-8 relative">
          {/* Action Bar (Boost) - Absolute positioned on desktop, relative on mobile */}
          <div className="md:absolute md:top-8 md:right-8 mb-4 md:mb-0 flex flex-col items-end gap-2">
            <BoostButton celebrityId={celebrity.id} locale={locale} />
          </div>
          ...
          <div className="mb-6">
            ...
            <RankingBadgeList rankings={rankings} locale={locale} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {celebrity.birthDate && (
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  {age && (
                    <div className="text-sm text-gray-500">{t('age', { age })}</div>
                  )}
                </div>
              </div>
            )}
            ...
            {celebrity.birthPlace && (
              <div className="flex items-start space-x-3">
                <div className="text-2xl">📍</div>
                <div>
                  <div className="font-medium text-gray-700">{t('born_place')}</div>
            ...
            {zodiac && (
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <div className="font-medium text-gray-700">{t('zodiac')}</div>
                  <div className="text-gray-900">
                    <Link
                      href={{ pathname: '/', query: { zodiac: zodiac.sign } }}
                      className="hover:text-blue-600 hover:underline transition-colors"
                    >
            ...
            {country && (
              <div className="flex items-start space-x-3">
                ...
                <div>
                  <div className="font-medium text-gray-700">{t('nationality')}</div>
                  <div className="text-gray-900">
                    <Link
